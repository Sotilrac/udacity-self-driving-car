...
class Layer:
    ...
    """
    def __init__(self, inbound_layers=list()):
        """
...
class Sigmoid(Layer):
    ...
    def backward(self):
        ...
        for layer in self.outbound_layers:
            self.gradients[self.inbound_layers[0]] += self.value * (1 - self.value) * layer.gradients[self]
...
def forward_and_backward(feed_dict, ideal_output, trainables=list()):
    """
